...
class LeCartCustom(LeBasecart):
	...
	def display_import_source(self):
		if self._notice['config']['add_new']:
			...
			if recent:
				...
				for _type in types:
					self._notice['process'][_type]['id_src'] = 0# recent['process'][_type]['id_src']
	...
	def convert_category_export(self, category, categories_ext):
		...
		category_data['created_at'] = time.strptime(category['date_added'], "%Y-%m-%d %H:%M:%S") if category['date_added'] else get_current_time()
		category_data['updated_at'] = time.strptime(category['date_added'], "%Y-%m-%d %H:%M:%S") if category['date_added'] else category_data['date_added']
	...
	def check_category_import(self, convert, category, categories_ext):
		id_imported = self.get_map_field_by_src(self.TYPE_CATEGORY, convert['categories_id'], convert['code'])
		return id_imported
...
#1
	def get_products_ext_export(self, products):
		...
		categories_id = duplicate_field_value_from_list(product_ext['data']['product_category'],'categories_id')
		categories_id_con = self.list_to_in_condition(categories_id)
		product_ext_rel_queries = {
			'category': {
				'type': "select",
				'query': "SELECT * FROM  _DBPRF_categories_description WHERE categories_id IN " + categories_id_con ,
			},
		}
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['code'] = product['products_mpn']
		products_categories = get_row_from_list_by_field(products_ext_data['product_category'],'products_id',product['products_id'])
		categories_id = products_categories['categories_id']
		categories = get_row_from_list_by_field(products_ext_data['category'],'categories_id',categories_id)
		
		product_data['type'] = categories['categories_name']
		product_description = get_row_from_list_by_field(products_ext_data['products_meta'],'products_id',product['products_id'])
		# product_data['is_child']
		# product_data['store_ids']
		# product_data['thumb_image']
		# product_data['images']
		product_data['name'] = product_description['products_name']
		# product_data['url_key']
		product_data['description'] = product_description['products_description']
		# product_data['meta_title']
		# product_data['meta_keyword']
		# product_data['meta_description']
		product_data['tags'] = product['products_model']
		product_data['is_in_stock'] = True if product['products_status']==1 else False
		...
		product_data['created_at'] = time.strptime(product['products_date_added'], "%Y-%m-%d %H:%M:%S") if product['products_date_added'] else get_current_time()
		product_data['updated_at'] = time.strptime(product['products_last_modified'],"%Y-%m-%d %H:%M:%S") if product['products_last_modified'] else product_data['created_at']
		product_data['price'] = product['products_msrp'] # product['products_price']
		product_data['cost'] = product['products_cost']
		product_description = get_list_from_list_by_field(products_ext_data['products_meta'], 'products_id',product['products_id'])
		...
		if product['products_image']!='':
			...
			product_data['thumb_image']['path'] = 'BSNIDESS1.39VANNPW-100.png'# product['products_image']

		if product_description:
			product_data['name'] = product_description[0]['products_name']
			product_data['description'] = product_description[0]['products_description']
		...
		product_categories = get_list_from_list_by_field(products_ext_data['product_category'], 'products_id',
		                                                 product['products_id'])
	...
	def get_customers_ext_export(self, customers):
		...
		zone_ids = duplicate_field_value_from_list(customers_ext['data']['address_book'], 'entry_state')
		customers_ext_rel_queries = {
			...
			'states': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_zones WHERE code_code IN " + self.list_to_in_condition(zone_ids),
			}
		}
	...
	def convert_customer_export(self, customer, customers_ext):
		# self.log(customer,'customer')
		# self.log(customers_ext,'customer_ext')
		...
		if address_books:
			for address_book in address_books:
				...
				address_data['id'] = address_book['customers_id']
				...
				if state_id:
					...
					if state:
						...
						address_data['state']['state_code'] = state['zone_country_id']
	...
	def get_orders_ext_export(self, orders):
		...
		country = duplicate_field_value_from_list(orders['data'], 'billing_country')
		zone_id = duplicate_field_value_from_list(orders['data'],'delivery_state')
		...
		orders_ext_queries = {
			...
			'countries': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_countries WHERE countries_name IN " + self.list_to_in_condition(country)
			},
			'customers':{
				'type':'select',
				'query':'select * from _DBPRF_customers where  customers_id IN' + self.list_to_in_condition(customers_id)
			},
			'zones': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_zones WHERE zone_code IN " + self.list_to_in_condition(zone_id)
			},
			'currencies': {
				'type': 'select',
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_data['tax']['amount'] = get_value_by_key_in_dict(orders_ext['data']['order_products'], 'products_tax', 0.0000)

		order_data['shipping']['title'] = 'Shipping'
		order_data['shipping']['amount'] = get_row_value_from_list_by_field(order_total,'sort_order',2,'value')

		order_data['discount']['title'] = 'Discount'
		order_data['discount']['amount'] = 0.0000

		order_data['total']['title'] = 'Total'
		order_data['total']['amount'] = get_row_value_from_list_by_field(order_total,'sort_order',4,'value')
		order_data['subtotal']['title'] = 'Total products'
		order_data['subtotal']['amount'] = get_row_value_from_list_by_field(order_total,'sort_order',1,'value')
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM  _DBPRF_customers WHERE customers_id =" + to_str(order_customer['id'])
		}
		customers = self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query)})
		if order_customer['id'] != '0':
			order_customer['code'] = 0
			order_customer['email'] = get_value_by_key_in_dict(customers['data'],'customers_email_address')# customers['data']['customers_email_address']
			order_customer['username'] = order['delivery_name']
			order_customer['first_name'] =  get_value_by_key_in_dict(customers['data'],'customers_firstname')
			order_customer['last_name'] =  get_value_by_key_in_dict(customers['data'],'customers_lastname')
		else:
			order_customer['username'] = order['delivery_name']
			order_customer['email'] = order['customers_email']
			user_name = order['delivery_name']
			order_customer['first_name'] = user_name.split()[0]
			order_customer['last_name'] = user_name.split()[-1]
		order_data['customer'] = order_customer
		customer_address = self.construct_order_address()
		if order_customer['id'] != '0':

			customer_address['first_name'] = get_value_by_key_in_dict(customers['data'],'customers_firstname')
			customer_address['last_name'] = get_value_by_key_in_dict(customers['data'],'customers_lastname')
		...
		customer_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_code', order['delivery_state'])
		if customer_state:
		...
		billing_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_code', order[
			'billing_state'])
		if billing_state:
		...
		order_products = get_list_from_list_by_field(orders_ext['data']['order_products'], 'orders_id', order['orders_id'])
		...
		for order_product in order_products:
			...
			query = {
				'type': 'select',
				'query': 'SELECT * FROM  _DBPRF_products_description WHERE products_name = "' + order_product['products_name']+'";'
			}
			product= self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query)})
			if product['data']:
				order_item['product']['id'] = product['products_id']
		...
		#
		self.log(order_data,'order_data')
