...
class LeCartWoocommerce(LeCartWordpress):
	...
	def display_config_source(self):
		...
		self._notice['src']['support']['attribute_map'] = True
...
#1
	...
	def category_import(self, convert, category, categories_ext):
		...
		category_data_query = self.create_insert_query_connector('terms', terms)
		...
		query = self.create_insert_query_connector('term_taxonomy',term_taxonomy)
		self.import_data_connector(query,'category')
		...
		for key,val in cate_termmeta_value.items():
			...
			query = self.create_insert_query_connector('termmeta',cate_termmeta)
	...
	def get_products_main_export(self):
		print('get_products_main_export')
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['code'] = 'dsfsdfsdfsd'
		product_data['sku'] = 'fsdfdsfsdf'
		...
		product_data['name'] = product['products_name']
...
#2
	def product_import(self, convert, product, products_ext):
		...
		product_posts = {
			...
			'post_excerpt':convert['description'],
			'post_status':'publish',
			'ping_status':'',
			'post_password':'',
		...
		product_data_query = self.create_insert_query_connector('posts', product_posts)
		product_id = self.import_data_connector(product_data_query,'product')
		if product_id:
			self.insert_map(self.TYPE_PRODUCT,convert['id'],product_id,value = convert['name'],lang=self._notice['target']['language_default'])
		else:
			return response_warning('Product ' + to_str(convert['id']) + ' import false.')

		data_keys = {
			'_edit_last':convert['updated_at'],
			'_sku':convert['sku'],
			'_regular_price':convert['price'],
			'_price':convert['price'],
			'_product_version':'4.8.0',
			'_stock':convert['qty'],
			'_sale_price':convert['special_price']['price'],
			'total_sale':convert['special_price']['price'],
			'_manage_stock':convert['manage_stock'],
			'_weight':convert['weight'],
			'_length':convert['length'],
			'_width':convert['width'],
			'_height':convert['height'],
			'_stock_status':convert['is_in_stock'],
			'_price':convert['price']
		}
...
#
		...
		query = self.create_insert_query_connector('wc_product_meta_lookup',product_meta_lookup)
		self.import_data_connector(query,'wc_product_meta_lookup')

		thumbnail_id = False
		if convert['thumb_image']['url'] or convert['thumb_image']['path']:
			image_process = self.process_image_before_import(convert['thumb_image']['url'], convert['thumb_image']['path'])
			image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(self.make_woocommerce_image_path(image_process['path'], self.TYPE_PRODUCT), self._notice['target']['config']['image_product'].rstrip('/')))

			if image_import_path:
				cate_image = self.remove_prefix_path(image_import_path, self._notice['target']['config']['image_product'])
				image_details = self.get_sizes(image_process['url'])
				thumbnail_id = self.wp_image(cate_image, image_details)
		...
		if thumbnail_id:
			product_posts = {
				...
				'guid':self._notice['target']['cart_url'].rstrip('/') + '/wp-content/uploads/' + self.make_woocommerce_image_path(image_process['path'], self.TYPE_PRODUCT),
				'menu_order':0,
			...
			product_data_query = self.create_insert_query_connector('posts', product_posts)
			image_post_id = self.import_data_connector(product_data_query,'product')

			meta_insert = {
				'post_id': image_post_id,
				'meta_key': '_wp_attached_file',
				'meta_value': self.make_woocommerce_image_path(image_process['path'], self.TYPE_PRODUCT)
			}
			...
			meta_insert = {
				...
				'meta_value': image_post_id
			}
			meta_query = self.create_insert_query_connector('postmeta', meta_insert)
			self.import_data_connector(meta_query, 'product')
		# self.insert_map(self.TYPE_PRODUCT,)
		return response_success(0)
...
#3
	def customer_import(self, convert, customer, customers_ext):
		...
		users = {
			...
			'user_status':convert['active'],
			'display_name':convert['username']
		}
		query = self.create_insert_query_connector('users', users)
		post_id = self.import_data_connector(query, 'users')
		if post_id:
			self.insert_map(self.TYPE_CUSTOMER,convert['id'],post_id,lang=self._notice['target']['language_default'])
		else:
			return response_warning('Users ' + to_str(convert['id']) + ' import false.')

		user_address = get_list_from_list_by_field(convert['address'],'id',convert['id'])
		user_address_row = get_row_from_list_by_field(user_address,'id',convert['id'])
		...
		usermeta = {
			...
			'billing_company':user_address_row['company'],
			'billing_address_1':user_address_row['address_1'],
			'billing_address_2':user_address_row['address_2'],
			'billing_city':user_address_row['city'],
			'billing_postcode':user_address_row['postcode'],
			'billing_country':user_address_row['country']['name'],
			'billing_state':user_address_row['state']['name'],
			...
			'shipping_company':user_address_row['company'],
			'shipping_address_1':user_address_row['address_1'],
			'shipping_address_2':user_address_row['address_2'],
			'shipping_city':user_address_row['city'],
			'shipping_post_code':user_address_row['postcode'],
			'shipping_country':user_address_row['country']['name'],
			'shipping_state':user_address_row['state']['name'],
		}
		for key,val in usermeta.items():
			data = {
				'user_id':post_id,
				'meta_key':key,
				'meta_value':val
			}
			query = self.create_insert_query_connector('usermeta', data)
			self.import_data_connector(query, 'usermeta')

		wc_customer_lookup = {
			'customer_id':convert['id'],
			'user_id':post_id,
			...
			'country':user_address_row['country']['name'],
			'postcode':user_address_row['postcode'],
			'city':user_address_row['city'],
			'state':user_address_row['state']['name']
		}
		query = self.create_insert_query_connector('wc_customer_lookup',wc_customer_lookup)
		self.import_data_connector(query,'wc_customer_lookup')
	...
	def before_order_import(self, convert, order, orders_ext):
		return response_success()
#4
	def import_product_order(self,product):
#posts
		post_data = {
			'post_author':'1',
			'post_date':get_current_time(),
			'post_title':product['product']['name'],
			'post_name':product['product']['name'],
			'post_modified':'',
			'post_parent':0,
			'post_status':'publish',
			'post_type':'product',
		}
		product_data_query = self.create_insert_query_connector('posts', post_data)
		product_id = self.import_data_connector(product_data_query,'product')
		if not product_id:
			return response_warning('Product ' + to_str(product_id) + ' import false.')
#postmeta
		post_metadata = {
			'_edit_last': '1',
			'_sku': product['product']['sku'],
			'_regular_price':product['original_price'],
			'_price': product['price'],
			'_product_version': '4.8.0',
			'_stock': product['qty'],
			'_sale_price': product['price'],
			'total_sale': product['total'],
			'_manage_stock': product['qty'],
			'_weight': '',
			'_length': '',
			'_width': '',
			'_height': '',
		...
		for key,val in post_metadata.items():
			postmeta = {
				'post_id': product_id,
				'meta_key': key,
				'meta_value': val
...
# post
		orders = {
			'post_author':'1',
			'post_date': convert['updated_at'] if convert['updated_at'] else time.time() ,       #  time.strptime(convert['date_updated'], "%Y-%m-%d %H:%M:%S"),
			'post_content':'order_post',
			'post_title':'order post',
		...
		query = self.create_insert_query_connector('posts', orders)
		post_id = self.import_data_connector(query, 'posts')
		if post_id:
			self.insert_map(self.TYPE_ORDER,convert['id'],post_id,lang=self._notice['target']['language_default'])
		else:
			return response_warning('Category ' + to_str(convert['id']) + ' import false.')
# postmeta
		products = convert['items']
		for p in products:
			product_id = self.import_product_order(p)
		num_items_sold = 0
		total_sales = 0
		for p in products:
			num_items_sold += int(p['qty'])
			total_sales += float(p['qty']) * float(p['price'])
		post_meta = {
			'_edit_last': '1',
			'_customer_user': convert['customer']['id'],
			'_order_total': total_sales,
			'_order_shipping_tax': 0,
			'_order_tax': 0,
			'_order_version': '4.8.0',
			'_prices_include_tax': convert['items'][0]['total'],
			'_created_via': 'admin',
			'_transaction_id': '',
			...
			'_billing_phone': convert['customer_address']['telephone'],
		...
		for key, val in post_meta.items():
			post = {
				'post_id': post_id,
				'meta_key': key,
				'meta_value': val
			}
			query = self.create_insert_query_connector('postmeta', post)
			self.import_data_connector(query, 'postmeta')
# insert product in order to database

		for product in products: # product in order

# woocommerce_order_items
			wc_woocommerce_order_items = {
				'order_item_name': product['product']['name'],
				'order_item_type': 'line_item',
				'order_id': post_id
			}
			query = self.create_insert_query_connector('woocommerce_order_items', wc_woocommerce_order_items)
			order_item_id = self.import_data_connector(query, 'woocommerce_order_items')
# woocommerce_order_itemeta
			line_tax_datas = {
				'subtotal': product['subtotal'],
				'total': product['total'],
			}
			wc_woocommerce_order_itemmeta = {
				'_product_id': product_id,
				'_qty': product['qty'],
				'_line_subtal': float(product['qty']) * float(product['price']),
				'_line_total': float(product['qty']) * float(product['price']),
				'_line_tax': 0,
				'_line_tax_data': php_serialize(line_tax_datas),
				'_reduce_stock': product['qty']
			}
			for key, val in wc_woocommerce_order_itemmeta.items():
				data = {
					'order_item_id': order_item_id,
					'meta_key': key,
					'meta_value': val
				}
				query = self.create_insert_query_connector('woocommerce_order_itemmeta', data)
				self.import_data_connector(query, 'woocommerce_order_itemmeta')
# order_product_lookup
			wc_order_product_lookup = {
				'order_item_id':order_item_id,
				'order_id':post_id,
				'product_id':product_id,
				'customer_id':convert['customer']['id'],
				'date_created':convert['created_at'],
				'product_qty':product['qty'],
				'product_net_revenue': float(product['qty']) * float(product['price']),
				'product_gross_revenue':float(product['qty']) * float(product['price'])
			}
			query = self.create_insert_query_connector('wc_order_product_lookup',wc_order_product_lookup)
			self.import_data_connector(query,'wc_order_product_lookup')
# wc_order_stats
		custommer_target = self.select_map(self._migration_id,self.TYPE_CUSTOMER,convert['customer']['id'])
		if  custommer_target:
			custommer_id = custommer_target['id_desc']
		else:
			custommer_id = 0
		wc_order_stats = {
			'order_id':post_id,
		...
		query = self.create_insert_query_connector('wc_order_stats',wc_order_stats)
		self.import_data_connector(query,'wc_order_stats')
		return response_success(0)
